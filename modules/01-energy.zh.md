...
- 持有 **30,000 ATOS** 可解锁 **50,000 TxEnergy 容量**,在 **24 小时**内线性回填。
...
采用默认参数(`TxEnergyHoldingThreshold = 30,000 ATOS`、`TxEnergyPerThreshold = 50,000`)时:
...
`TxEnergyMaxAccrueWindow` 默认为 86,400(24 小时)。先乘后除的运算顺序很关键:朴素的 `(cap / window) × elapsed` 对任何不足一个窗口的间隔都会被整数运算截断为零(`50000 / 86400 == 0`)。所有中间乘积都使用饱和运算,因此治理层的重新调参永远不会让 `uint64` 回绕溢出。

**实例演算(TxEnergy)。** Alice 持有 30,000 ATOS,故 `cap = 50,000`。速率 = 50,000 / 86,400 ≈ **0.5787 能量/秒**。

- 从空开始。1 小时(3,600 秒)后:`add = 50,000 × 3,600 / 86,400 = 2,083`。累积 ≈ 2,083。
- 12 小时后:`add = 50,000 × 43,200 / 86,400 = 25,000`。累积 = 25,000(容量的一半)。
- 24 小时后:累积触及 50,000 上限并停止。持有更久也不会溢满。
...
修复前的代码在发送方一侧重复扣减,在*每一笔*转账中悄悄地从快照里削掉一份 `moved` 的合格余额——对于一笔 30,000 ATOS 的划转,那恰好是一个阈值区块,即每次发送容量下降 50,000 能量。这就是测试网账户 `0x30F288…` 上生产环境 "5万 ATOS 凭空消失" 的特征表现。修复方案在 `keeper/send_restriction.go` 中有内联文档说明。
...
**向上取整,以整数阈值区块计。** 采用默认值时,委托从 1 到 50,000 之间的任意数量能量都会锁定整整 **30,000 ATOS**。委托 50,001 则锁定 **60,000 ATOS**。钱包 UI 必须把这一点呈现出来——否则用户会惊讶于出借"一点点"能量竟冻结了整整一个区块的 ATOS。
...
| `tx_energy_per_threshold` | 50,000 | 每个区块的 TxEnergy 单位数 |
